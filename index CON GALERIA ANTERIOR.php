...
<div class="container" style="padding:15px 0;">
  ...
  <!-- GALERIA DE FOTOS -->
	...
	{
		echo '<div class="col-md-12 col-sm-12 col-xs-6" align="center"><a href="franquicias.php?idf='.$grp1dest['idfranquicia'].'&nom='.$grp1dest['nombre'].'"><img src="img/logos-clientes/'.$grp1dest['logo'].'" alt="'.$grp1dest['nombre'].'" title="'.$grp1dest['nombre'].'" class="img-responsive clientes"></a></div>';
	}?>
              </div>
  ...
  <div id="carousel-example-generic2" class="carousel slide" data-ride="carousel" style="padding-top:15px;">

    <!-- Wrapper for slides -->
    <div class="carousel-inner" role="listbox">
      <? $fradest = mysql_query("Select DISTINCT (idfranquicia) From franquicias Where destacado='1' and estado='1'",$conexion) or die("Problemas en el select:".mysql_error());
		$totdest = mysql_num_rows($fradest);
		$total = round(($totdest/8));
		//echo $total;
		
		for ($i = 1; $i <= $total; $i++) {
		if ($i=='1') { $valest=' active'; } else { $valest=' ';}
echo '<div class="item'.$valest.'" align="center">';
	echo '<div class="row" style="width:90%;">';
	$grp1des = mysql_query("Select DISTINCT * From franquicias Where destacado='1' and estado='1' Order by RAND() LIMIT 4",$conexion) or die("Problemas en el select:".mysql_error());
	...
	echo '<hr>';
	echo '<div class="row" style="width:90%;">';
	$grp2des = mysql_query("Select DISTINCT * From franquicias Where destacado='1' and estado='1' Order by RAND() LIMIT 4",$conexion) or die("Problemas en el select:".mysql_error());
	while ($grp2dest=mysql_fetch_array($grp2des))
	{
		echo '<div class="col-md-3 col-sm-3 col-xs-6" align="center"><a href="franquicias.php?idf='.$grp1dest['idfranquicia'].'&nom='.$grp1dest['nombre'].'"><img src="img/logos-clientes/'.$grp2dest['logo'].'" alt="'.$grp1dest['nombre'].'" title="'.$grp1dest['nombre'].'" class="img-responsive clientes"></a></div>';
	}
	echo '</div>';
echo '</div>';
		} ?>
...
<div style="padding:30px 0; background:#fc9a00; color:#fff;" align="center">
  ...
  <div class="container" align="center">
    <div class="row" style="width:70%;">
      <div class="col-md-10 col-sm-10 col-xs-9"><input type="email" class="form-control" placeholder="Ingresa tu correo electr&oacute;nico"></div>
      <div class="col-md-2 col-sm-2 col-xs-3" align="left"><input type="submit" name="button" id="button" value="Registrarme" class="btn btn-danger"></div>
    </div>
  </div>
</div>
<!--fin newsletter-->
